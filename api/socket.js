...
const ioHandler = (req, res) => {
  ...
  // Handle DELETE - delete message or poll
  if (req.method === 'DELETE') {
    const { roomId, type, messageId, pollId } = req.body;
    ...
    if (type === 'message' && messageId !== undefined) {
      // Delete message by ID
      const initialLength = data.messages.length;
      data.messages = data.messages.filter(m => m.id !== messageId);
      roomData.set(roomId, data);
      
      console.log(`[DELETE MESSAGE] Removed message ${messageId} from room ${roomId}`);
      res.status(200).json({ success: true, deleted: initialLength > data.messages.length });
      return;
    }
    
    if (type === 'poll' && pollId !== undefined) {
      // Delete poll by ID
      const initialLength = data.polls.length;
      data.polls = data.polls.filter(p => p.id !== pollId);
      roomData.set(roomId, data);
      
      console.log(`[DELETE POLL] Removed poll ${pollId} from room ${roomId}`);
      res.status(200).json({ success: true, deleted: initialLength > data.polls.length });
      return;
    }
