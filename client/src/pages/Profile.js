      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-12 xl:px-16 py-8">
              ...
              <div className="relative group">
                ...
                <div className="relative">
                  <img
                    src={profilePicture}
                    alt="Profile"
                    className="w-32 h-32 rounded-full object-cover border-4 border-white shadow-2xl transition-transform duration-300 group-hover:scale-105"
                  />
                ...
                <button
                  ...
                  className="absolute bottom-0 right-0 bg-white hover:bg-gray-100 disabled:opacity-50 text-blue-600 p-2.5 rounded-full shadow-lg transition-all duration-200 hover:scale-110"
                >
            ...
            <div className="flex-1 w-full text-white">
                ...
                <div className="flex-1 min-w-0">
                  {isEditMode ? (
                    <input
                      ...
                      className="text-3xl font-bold bg-white/20 backdrop-blur-sm text-white placeholder-white/70 border border-white/30 rounded-lg px-4 py-2 focus:ring-2 focus:ring-white/50 focus:border-transparent transition-all duration-300 w-full"
                  ...
                  {isEditMode ? (
                    <>
                      <button
                        ...
                        disabled={isSaving}
                        className="bg-green-500 hover:bg-green-600 disabled:opacity-50 text-white px-4 py-2 rounded-lg transition-all duration-200 font-medium shadow-lg hover:shadow-xl hover:scale-105"
                        title="Save Changes"
                      >
                        {isSaving ? (
                      ...
                      <button
                        ...
                        disabled={isSaving}
                        className="bg-white/20 hover:bg-white/30 disabled:opacity-50 text-white px-4 py-2 rounded-lg transition-all duration-200 font-medium backdrop-blur-sm"
                        title="Cancel"
                      >
                        <div className="flex items-center gap-2">
                          <X className="w-5 h-5" />
                    ...
                    <button
                      onClick={handleEditToggle}
                      className="bg-white hover:bg-gray-100 text-blue-600 px-6 py-2 rounded-lg font-medium transition-all duration-200 flex items-center space-x-2 shadow-lg hover:shadow-xl hover:scale-105"
                    >
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  {isEditMode && (
                    <span className={`text-sm ${editForm.bio.length > 500 ? 'text-red-300' : 'text-blue-100'}`}>
                ...
                {isEditMode ? (
                  <textarea
                    ...
                    className={`w-full min-h-[100px] p-4 border rounded-lg focus:ring-2 focus:ring-white/50 focus:border-transparent bg-white/20 backdrop-blur-sm text-white placeholder-white/70 transition-all duration-300 resize-vertical ${
                  ...
                  <div className="w-full min-h-[100px] p-4 border border-white/20 rounded-lg bg-white/10 backdrop-blur-sm text-white transition-all duration-300">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {stats.map((stat, index) => (
            <div 
              key={index} 
              className="bg-white dark:bg-gray-800/50 backdrop-blur-xl border border-gray-200 dark:border-gray-700 rounded-xl shadow-md hover:shadow-xl p-6 transition-all duration-300 ease-in-out hover:scale-105 hover:border-gray-300 dark:hover:border-gray-600 group"
            >
              <div className={`inline-flex p-3 rounded-lg bg-gradient-to-br ${stat.gradient} mb-3 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                {stat.icon}
              </div>
        ...
        <div className="grid gap-8">
          ...
          <div className="bg-white dark:bg-amber-900/20 backdrop-blur-xl border border-gray-200 dark:border-amber-700/50 rounded-xl shadow-md hover:shadow-lg p-6 transition-all duration-300 ease-in-out">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-amber-100 flex items-center gap-3">
                <div className="p-2 bg-gradient-to-br from-amber-500 to-amber-600 rounded-lg shadow-lg">
                  <Calendar className="w-5 h-5 text-white" />
                </div>
                Scheduled Meetings
              </h2>
              <span className="text-sm text-gray-500 dark:text-amber-400 font-medium bg-gray-100 dark:bg-amber-900/30 px-3 py-1 rounded-full">
            ...
            {scheduledMeetings.length > 0 ? (
              <div className="space-y-3">
                {scheduledMeetings.map((meeting) => (
                  <div
                    key={meeting.id}
                    className="flex items-center justify-between p-4 bg-gray-50 dark:bg-amber-800/20 rounded-lg border border-gray-200 dark:border-amber-600/30 hover:bg-gray-100 dark:hover:bg-amber-800/30 hover:border-gray-300 dark:hover:border-amber-600/50 hover:shadow-md transition-all duration-300 group"
                  >
                    <div className="flex-1">
                      <h3 className="font-semibold text-gray-900 dark:text-amber-50 group-hover:text-blue-600 dark:group-hover:text-amber-200 transition-colors">
                        {meeting.title}
                      </h3>
                      <div className="flex items-center gap-4 mt-2">
                        <span className="text-sm text-gray-600 dark:text-amber-300 flex items-center gap-1.5">
                          <Clock className="w-4 h-4" />
                          {formatScheduledDateTime(meeting.scheduledAt)}
                        </span>
                      </div>
                      {meeting.description && (
                        <p className="text-sm text-gray-500 dark:text-amber-400 mt-2 line-clamp-1">
                          {meeting.description}
                        </p>
                      )}
                    </div>
                    <button
                      ...
                      className="p-2.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-all duration-200 disabled:opacity-50 hover:scale-110"
              ...
              <div className="text-center py-12 text-gray-500 dark:text-amber-400">
                <div className="bg-gradient-to-br from-amber-500 to-amber-600 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 shadow-lg">
                  <Calendar className="w-8 h-8 text-white" />
                </div>
                <p className="font-semibold text-lg mb-1">No scheduled meetings</p>
                <p className="text-sm">Schedule a meeting from the home page</p>
              </div>
            )}
          </div>

          {/* Call History Section with Premium Design */}
          <div className="bg-white dark:bg-indigo-900/20 backdrop-blur-xl border border-gray-200 dark:border-indigo-700/50 rounded-xl shadow-md hover:shadow-lg p-6 transition-all duration-300 ease-in-out">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-indigo-100 mb-6 flex items-center gap-3">
            ...
            <div className="overflow-x-auto">
              ...
              {callHistory.length === 0 && (
                <div className="text-center py-12 text-gray-500 dark:text-indigo-400">
                  ...
                  <p className="font-semibold text-lg mb-1">No meeting history</p>
                  <p className="text-sm">Your completed meetings will appear here</p>
                </div>
              )}
            </div>
          </div>
